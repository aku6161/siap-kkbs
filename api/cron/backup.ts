 ...
 * Also accessible at POST /api/cron/backup for manual triggering.
 *
 * This handler:
 *   1. Fetches all complaints from the database.
 *   2. Converts records into CSV format with full details.
 *   3. Sends CSV to Google Apps Script which saves it to Google Drive folder 1f2VTd_dug6ANOkyRqHtC7LaNcBJWoU28.
 *   4. Keeps the 2 most recent backups (latest 2 weeks) and deletes older backups.
 */
export async function runBackup(): Promise<{ success: boolean; message: string; fileName?: string; fileUrl?: string }> {
  ...
  // Format filename: siap_backup_YYYY-MM-DD_HHmm.csv
  ...
  try {
    const response = await fetch(gasUrl, {
      ...
      body: JSON.stringify({
        ...
        fileName,
        content: csvContent,
        maxBackups: 2, // Kekal 2 fail sandaran mingguan terkini (2 minggu), selebihnya dipadam
      }),
      redirect: 'follow',
    });
